...
class SubscriptoinDate {
  ...
  SubscriptoinDate(
      {required this.date, required this.status, required this.subscriptionId});
}

SubscriptoinDate mapSubscriptoinDate(dynamic payload) {
  DateTime dateTime = payload["date"] != null && payload["date"] != false
      ? getParsableDate(payload["date"])
      : DefaultInvalidDate;

  return SubscriptoinDate(
      subscriptionId: payload["subscription_id"] ?? -1,
      status: payload["status"] != null && payload["status"] != false
          ? payload["status"]
          : "",
      date: dateTime);
}
