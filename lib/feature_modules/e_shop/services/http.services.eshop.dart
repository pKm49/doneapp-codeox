...
class EshopHttpService {
  ...
  Future<bool> addToCart(String mobile, int mealId, int quantity) async {
    try {
      ...
      AppHttpResponse response =
          await postRequest('$EshopHttpRequestEndpoint_AddToCart', params);
  ...
  Future<bool> removeFromCart(String mobile, int mealId, int quantity) async {
    try {
      ...
      AppHttpResponse response =
          await postRequest('$EshopHttpRequestEndpoint_RemoveFromCart', params);
  ...
  Future<bool> emptyFromCart(String mobile) async {
    try {
      ...
      AppHttpResponse response =
          await postRequest('$EshopHttpRequestEndpoint_EmptyCart', params);
  ...
  Future<PaymentData> checkout(
      String mobile, int addressId, int shiftId, String couponCode) async {
    try {
      Map<String, dynamic> params = {};
      params["mobile"] = mobile;
      ...
      AppHttpResponse response =
          await postRequest('$EshopHttpRequestEndpoint_Checkout', params);
  ...
  Future<List<GeneralItem>> getShiftList() async {
    try {
      ...
      tempAddressList.add(mapGeneralItem(
          {"id": -1, "name": "Select Time", "arabic_name": "حدد الوقت"}));
      if (response.statusCode == 200 && response.data != null) {
        for (var i = 0; i < response.data.length; i++) {
