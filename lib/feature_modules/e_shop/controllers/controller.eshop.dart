...
import 'package:doneapp/feature_modules/e_shop/services/http.services.eshop.dart';
...
class EshopController extends GetxController {
  ...
  Future<void> getCart() async {
    ...
    if (tMobile != null && tMobile != '') {
      if (!isCartDetailsFetching.value) {
        ...
        var eshopHttpService = new EshopHttpService();
        cart.value = await eshopHttpService.getCartDetails(tMobile);
    ...
    if (tMobile != null && tMobile != '') {
      if (!isAddingToOrRemoveFromCart.value) {
        ...
        if (isAdd) {
          ...
          isSuccess = await eshopHttpService.removeFromCart(
              tMobile, mealId, quantity * -1);
        }
        getCart();
      }
    }
  ...
  Future<void> getMealCategories() async {
    if (!isCategoriesFetching.value && !isMealsLoading.value) {
      ...
      var eshopHttpService = new EshopHttpService();
      mealCategories.value = await eshopHttpService.getMealCategories();
  ...
  void createOrder() async {
    ...
    if (mobile != null && mobile != "") {
      ...
      PaymentData tPaymentData = await planPurchaseHttpService.checkout(mobile,
          addressId.value, shiftId.value, couponCodeController.value.text);
  ...
  Future<void> emptyCart() async {
    ...
    if (tMobile != null && tMobile != '') {
      if (!isEmptyingCart.value) {
        ...
        var eshopHttpService = new EshopHttpService();
        bool isSuccess = await eshopHttpService.emptyFromCart(tMobile);
  ...
  Future<void> getShiftList() async {
    ...
    shiftList.value = await addressHttpService.getShiftList();
