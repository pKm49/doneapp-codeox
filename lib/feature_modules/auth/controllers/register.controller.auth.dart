...
import 'package:dietdone/shared_module/constants/available_sources.shared.constant.dart';
...
class RegisterController extends GetxController {
  ...
  handleRegistration(String tSource) async {
    ...
    bool isSuccess = await authHttpService.register(RegisterCredential(
        ...
        gender: gender.value,
        height: double.parse(
            heightTextEditingController.value.text.toString().trim()),
        weight: double.parse(
            weightTextEditingController.value.text.toString().trim()),
        source: tSource,
        ...
        other_source:
            tSource != "" ? otherSourceTextEditingController.value.text : ""));
    isRegisterSubmitting.value = false;
    if (isSuccess) {
      var sharedPreferences = await SharedPreferences.getInstance();
      sharedPreferences.setString("mobile", mobile.value);
      showSnackbar(Get.context!, "account_created".tr, "info");
      final sharedController = Get.find<SharedController>();
      sharedController.fetchUserData(
          AppRouteNames.dislikeAuditRoute, mobile.value);
    }
  }
}
