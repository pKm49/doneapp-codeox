...
class AuthHttpService {
  Future<bool> login(LoginCredential loginCredentials) async {
    try {
      ...
      AppHttpResponse response =
          await getRequest(AuthHttpRequestEndpoint_Login, params);
  ...
  Future<bool> register(RegisterCredential registerCredentials) async {
    try {
      AppHttpResponse response = await postRequest(
          AuthHttpRequestEndpoint_Register, registerCredentials.toJson());
      if (response.statusCode != 200) {
        showSnackbar(Get.context!, response.message, "error");
      }
      return response.statusCode == 200;
  ...
  Future<bool> resetPassword(String mobile, String newPassword, String gender,
      String birthday, String email) async {
    try {
      AppHttpResponse response =
          await patchRequest(AuthHttpRequestEndpoint_ResetPassword, {
        "mobile": mobile,
      ...
      showSnackbar(Get.context!, response.message,
          response.statusCode != 200 ? "error" : "info");
  ...
  Future<List<MealCategory>> getMealsByDay(String day) async {
    try {
      ...
      params["day"] = day;
      AppHttpResponse response =
          await getRequest(AuthHttpRequestEndpoint_GetMealsByDay, params);
