...
class AddressController extends GetxController {
  ...
  Rx<TextEditingController> apartmentNumberTextEditingController =
      TextEditingController().obs;
  ...
  Rx<TextEditingController> jedhaTextEditingController =
      TextEditingController().obs;
  Rx<TextEditingController> nicknameTextEditingController =
      TextEditingController().obs;
  ...
  Future<void> getAreas() async {
    if (areas.isEmpty) {
      ...
      areas.value = await addressHttpService.getAreas();
  ...
  Future<void> getBlocks(int tAreaId) async {
    ...
    var addressHttpService = new AddressHttpService();
    blocks.value = await addressHttpService.getBlocks(tAreaId);
  ...
  Future<void> deleteAddress(int id) async {
    List<Address> tAddressList =
        customerAddressList.where((p0) => p0.id == id).toList();
  ...
  Future<void> changeAuditAddress(int id) async {
    ...
    if (tAddressList.isNotEmpty) {
      ...
      apartmentNumberTextEditingController.value.text =
          currentAddress.value.apartmentNo == -1
              ? ""
              : currentAddress.value.apartmentNo.toString();
      houseNumberTextEditingController.value.text =
          currentAddress.value.houseNumber == -1
              ? ""
              : currentAddress.value.houseNumber.toString();
      floorNumberTextEditingController.value.text =
          currentAddress.value.floorNumber == -1
              ? ""
              : currentAddress.value.floorNumber.toString();
    ...
    Get.toNamed(AppRouteNames.addressAuditRoute,
        arguments: [VALIDADDRESSAUTHOR_MODES.author_address, ""]);
  }
  ...
  Future<void> auditAddress() async {
    if (streetTextEditingController.value.text != "" &&
      ...
      if (mobile != null && mobile != "") {
        ...
        var addressHttpService = new AddressHttpService();
        bool isSuccess = await addressHttpService.auditAddress(
            Address(
              id: currentAddress.value.id,
              name: nicknameTextEditingController.value.text,
              comments: commentsTextEditingController.value.text,
              apartmentNo:
                  apartmentNumberTextEditingController.value.text.toString(),
              houseNumber:
                  houseNumberTextEditingController.value.text.toString(),
              floorNumber:
                  floorNumberTextEditingController.value.text.toString(),
