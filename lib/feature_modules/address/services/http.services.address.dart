...
class AddressHttpService {
  ...
  Future<bool> auditAddress(
      Address address, String mobile, bool isCreate) async {
    try {
      if (isCreate) {
        AppHttpResponse response = await postRequest(
            AddressHttpRequestEndpoint_Address, address.toJsonForPost(mobile));
        ...
        AppHttpResponse response = await patchRequest(
            AddressHttpRequestEndpoint_Address, address.toJsonForPatch(mobile));
        if (response.statusCode != 200) {
          showSnackbar(Get.context!, response.message, "error");
        }
        return response.statusCode == 200;
