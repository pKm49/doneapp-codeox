...
List<Map<String, List<int>>> mapSubscriptoinMealConfigForSubmit(
    SubscriptoinMealConfig subscriptoinMealConfig) {
  List<Map<String, List<int>>> mealConfigList = [];
  Map<String, List<int>> mealConfig = {};

  for (var element in subscriptoinMealConfig.meals) {
    mealConfig.addAll(
        {element.id.toString(): element.items.map((e) => e.id).toList()});
  }
