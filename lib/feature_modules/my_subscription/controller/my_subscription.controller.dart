...
import 'package:dietdone/feature_modules/my_subscription/services/http.my_subscription.service.dart';
import 'package:dietdone/shared_module/constants/app_route_names.constants.shared.dart';
import 'package:dietdone/shared_module/constants/valid_subscription_day_status.constants.shared.dart';
...
class MySubscriptionController extends GetxController {
    ...
    if (!isSubscriptionDatesLoading.value) {
      ...
      if (tMobile != null && tMobile != '') {
        ...
        subscriptionDates.value =
            await mySubsHttpService.getSubscriptionDates(tMobile);
  ...
  void setCurrentMonth() {
    if (subscriptionDates.isNotEmpty) {
      ...
      subscriptionDates.forEach((element) {
        ...
        if (!doesContainDate(subscriptionMonths,
            DateTime(element.date.year, element.date.month, 1))) {
          subscriptionMonths
              .add(DateTime(element.date.year, element.date.month, 1));
        }
      });
  ...
  bool isSubscriptionDay(DateTime firstWeekDay) {
    return subscriptionDates
        .where((p0) => isSameDay(p0.date, firstWeekDay))
        .toList()
        .isNotEmpty;
  }
  ...
  Future<void> getMealsByDate(
      DateTime tSelectedDay, bool isNavigationRequired) async {
    print("getMealsByDate triggered");
    print(tSelectedDay);
      ...
      if (isSubscriptionDay(tSelectedDay)) {
        ...
        if (getDayStatus(tSelectedDay) != VALIDSUBSCRIPTIONDAY_STATUS.offDay &&
          ...
          if (tMobile != null && tMobile != '') {
            ...
            var mySubsHttpService = MySubsHttpService();
            subscriptoinMealConfig.value = await mySubsHttpService
                .getMealsByDay(tMobile, f.format(selectedDate.value));
  ...
  bool isSetMealsEnabled(bool isNavigateBack, bool isFromBackbutton) {
    ...
    if (getDayStatus(selectedDate.value) ==
        ...
        getDayStatus(selectedDate.value) ==
    ...
    if (getDayStatus(selectedDate.value) ==
        ...
        selectedMealConfig.value.meals
            .where((element) => element.items.isNotEmpty)
            .toList()
            .isEmpty &&
        !isFromBackbutton) {
      showSnackbar(
          Get.context!, "please_select_meals_for_all_categories".tr, "error");
      return false;
    }
    ...
    if (getDayStatus(selectedDate.value) ==
        ...
        selectedMealConfig.value.meals
                .where((element) => element.items.length == element.itemCount)
                .toList()
                .length !=
  ...
  Future<void> setMealsByDate(
      bool isNavigateBack, int subscriptionId, bool isFromBackbutton) async {
      ...
      if (getDayStatus(selectedDate.value) ==
        ...
        if (tMobile != null && tMobile != '') {
          if (subscriptionId != -1) {
            ...
            final f = new DateFormat('yyyy-MM-dd');
            var mySubsHttpService = MySubsHttpService();
            bool isSuccess = await mySubsHttpService.saveMealsByDay(
                subscriptionId,
                selectedMealConfig.value,
                f.format(selectedDate.value));
  ...
  void initializeMealSelection(String date) {
    try {
      ...
      selectedMealConfig.value = SubscriptoinMealConfig(
          date: date, recommendedCalories: recommendedCalories, meals: meals);
    } catch (e, st) {
      print(e);
      print(st);
    }
  ...
  addOrRemoveMeal(int index, int categoryId, int mealId, int count) {
    print("addOrRemoveMeal");
    print(mealId);
    print(count);
    double currentSelectedCalories =
        selectedMealConfig.value.recommendedCalories;
    List<SubscriptoinDailyMeal> meals = [];
    for (var element in subscriptoinMealConfig.value.meals) {
      if (element.id != categoryId) {
        ...
        List<SubscriptoinDailyMealItem> mealItem =
            element.items.where((element) => element.id == mealId).toList();
        if (mealItem.isEmpty) {
          meals.add(SubscriptoinDailyMeal(
              name: element.name,
              id: element.id,
          ...
          if (count < 0) {
            List<SubscriptoinDailyMealItem> alreadySelectedMeals =
                getAlreadySelectedMeals(element.id);
            final item =
                alreadySelectedMeals.firstWhere((e) => element.id != mealId);
    ...
    selectedMealConfig.value = SubscriptoinMealConfig(
        ...
        recommendedCalories: currentSelectedCalories,
        meals: meals);

    if (selectedMealConfig.value.meals.length > (index + 1) &&
        isMealMaximumCountReached(categoryId)) {
  ...
  List<SubscriptoinDailyMealItem> getAlreadySelectedMeals(int categoryId) {
    if (selectedMealConfig.value.meals
        .where((element) => element.id == categoryId)
        .toList()
        .isNotEmpty) {
      return selectedMealConfig.value.meals
          .where((element) => element.id == categoryId)
          .toList()[0]
          .items;
    } else {
  ...
  getMealSelectedCount(int categoryId, int mealId) {
    final List<SubscriptoinDailyMeal> meals = selectedMealConfig.value.meals
        .where((element) => element.id == categoryId)
        .toList();
  ...
  isMealMaximumCountReached(int categoryId) {
    ...
    selectedMeals = selectedMealConfig.value.meals
        .where((element) => element.id == categoryId)
        .toList();
    originalMeals = subscriptoinMealConfig.value.meals
        .where((element) => element.id == categoryId)
        .toList();
  ...
  Future<void> freezeSubscription(DateTime dateTime, bool isFreeze) async {
    if (!isFreezing.value) {
      ...
      if (subscriptionId != -1) {
        ...
        var mySubsHttpService = MySubsHttpService();
        bool isSuccess = await mySubsHttpService.freezeSubscriptionDays(
            subscriptionId, frozenDays, isFreeze);
        if (isSuccess) {
          Get.back();
          if (isFreeze) {
  ...
  removeSelectionPerCategoryAll(
    int categoryId,
  ) {
    double currentSelectedCalories =
        subscriptoinMealConfig.value.recommendedCalories;
    List<SubscriptoinDailyMeal> meals = [];
    for (var element in subscriptoinMealConfig.value.meals) {
      if (element.id != categoryId) {
        meals.add(SubscriptoinDailyMeal(
    ...
    subscriptoinMealConfig.value = SubscriptoinMealConfig(
        ...
        recommendedCalories: subscriptoinMealConfig.value.recommendedCalories,
        meals: meals);
  }
  ...
  Future<void> unfreezeAndSaveMeals() async {
    if (!isFreezing.value) {
      ...
      if (subscriptionId != -1) {
        ...
        var mySubsHttpService = MySubsHttpService();
        bool isSuccess = await mySubsHttpService.freezeSubscriptionDays(
            subscriptionId, frozenDays, false);
        if (isSuccess) {
          final SharedPreferences prefs = await SharedPreferences.getInstance();
          final String? tMobile = prefs.getString('mobile');
          if (tMobile != null && tMobile != '') {
            ...
            if (subscriptionId != -1) {
              ...
              showSnackbar(
                  Get.context!, "couldnt_load_profiledata".tr, "error");
              showSnackbar(Get.context!, "login_message".tr, "error");
              Get.offAllNamed(AppRouteNames.loginRoute);
            }
          } else {
