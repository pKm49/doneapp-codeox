import 'package:dietdone/feature_modules/profile/models/referral_data.profile.model.dart';
import 'package:dietdone/feature_modules/profile/services/http.profile.service.dart';
  ...
  getUserData() async {
    ...
    if (tMobile != null && tMobile != '') {
      ...
      if (userData.value.id == -1) {
        ...
        selectedDOB.value = userData.value.birthday == ""
            ? DateTime.now()
            : getParsableDate(userData.value.birthday);
  ...
  updateDislikes(bool isRegisterComplete) async {
    if (!isDislikesUpdating.value) {
      ...
      if (tMobile != null && tMobile != '') {
        ...
        var profileHttpService = ProfileHttpService();
        bool isSuccess =
            await profileHttpService.updateDislikes(dislikes, tMobile);
        ...
        if (isSuccess) {
          ...
          showSnackbar(
              Get.context!, "dislikes_updated_successfully".tr, "info");
        }
      } else {
        showSnackbar(Get.context!, "couldnt_load_profiledata".tr, "error");
        showSnackbar(Get.context!, "login_message".tr, "error");
  ...
  updateAllergies(bool isRegisterComplete) async {
    if (!isAllergiesUpdating.value && allergies.isNotEmpty) {
      ...
      if (tMobile != null && tMobile != '') {
        ...
        var profileHttpService = ProfileHttpService();
        bool isSuccess =
            await profileHttpService.updateAllergies(allergies, tMobile);
        ...
        if (isSuccess) {
          ...
          Get.back();
          showSnackbar(
              Get.context!, "allergies_updated_successfully".tr, "info");
        }
      } else {
        showSnackbar(Get.context!, "couldnt_load_profiledata".tr, "error");
        showSnackbar(Get.context!, "login_message".tr, "error");
